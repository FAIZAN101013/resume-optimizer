...
export default function Tracker() {
  ...
  const handleAddJob = async (form) => {
    const { data, error } = await supabase.from('jobs').insert([{
      ...
      date: form.date, company_email: form.companyEmail,
      is_referral: form.isReferral, referral_email: form.referralEmail,
      notes: form.notes,
    }]).select()
    if (error) console.error(error)
  ...
  const handleEditSave = async (updatedJob) => {
    const { error } = await supabase.from('jobs').update({
      company: updatedJob.company, role: updatedJob.role,
      status: updatedJob.status, date: updatedJob.date,
      company_email: updatedJob.companyEmail, is_referral: updatedJob.isReferral,
      referral_email: updatedJob.referralEmail, notes: updatedJob.notes,
    }).eq('id', updatedJob.id)
    if (error) console.error(error)
...
/>
      ...
      <StatsBar counts={counts} activeFilter={activeFilter} onFilter={setActiveFilter} />

      <SearchBar value={search} onChange={setSearch} onClear={() => setSearch('')} />

      <FilterTabs activeFilter={activeFilter} onFilter={setActiveFilter} counts={counts} />
      ...
      {loading ? (
        ...
        <div className="text-center py-16 text-gray-600">
          <Briefcase className="w-8 h-8 mx-auto mb-3 opacity-30" strokeWidth={1.5} />
          <p className="text-sm">
            {search ? `No results for "${search}"` : 'No jobs found. Add your first application!'}
          </p>
        </div>
      ) : (
      ...
      {selectedJob && modalMode === 'view' && (
        <JobViewModal
          job={selectedJob}
          onClose={() => { setSelectedJob(null); setModalMode(null) }}
