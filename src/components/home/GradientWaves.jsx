...
// The shaders are GLSL ES 3.00, which needs WebGL2. On a WebGL1-only context
// they fail to compile and the canvas renders black, so check before starting.
...
const GradientWaves = ({
  ...
  useEffect(() => {
    ...
    // Nothing to fall back to but an empty container, which is exactly what
    // the parent's gradient wash already covers.
    ...
    // Someone who asked the OS for less motion should not get an animated
    // raymarched ocean. One frame, then stop.
    if (prefersReducedMotion()) {
      program.uniforms.iTime.value = 12;
      program.uniforms.uEnableMouse.value = false;
      renderer.render({ scene: mesh });
    ...
    // Scrolled out of view or tabbed away means no GPU work at all.
