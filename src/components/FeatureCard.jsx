...
export default function FeatureCard({
  ...
  return (
    <Link
      to={to}
      className={`group flex flex-col p-6 rounded-2xl bg-white border border-gray-200 dark:bg-white/[0.02] dark:border-white/[0.06] shadow-xl transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl ${v.hover}`}
    >
      <div className={`w-10 h-10 rounded-xl flex items-center justify-center mb-5 ${v.icon}`}>
        <Icon className="w-5 h-5" strokeWidth={1.75} />
      </div>

      <h3 className="text-gray-900 dark:text-white font-semibold text-lg mb-2 group-hover:text-gray-800 dark:group-hover:text-white/90 transition-colors">
        {title}
      </h3>
